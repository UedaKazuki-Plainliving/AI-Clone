...
async function fetchSettings() {
  try {
    ...
    if (response.ok) {
      ...
      document.getElementById('setting-provider').value = data.provider || 'mock';
      ...
      if (data.provider === 'mock') {
        ...
        badge.textContent = `${data.provider.toUpperCase()} API 稼働中`;
...
function initPresets() {
  ...
  const presets = [
    {
      systemName: 'ホテル予約サイト',
      surveyInput: `【利用者ヒアリングシート回収データ（上田様・51歳男性・ITリテラシー高）】
- 利用目的：出張時のホテル予約
- 良かった点：シンプルであり、おすすめのプランが最初に表示されること。
- 不満点（ペインポイント）：宿泊日は決まっているのに、その日に予約可能なプランだけをフィルタリングして表示してくれない。空き状況を確認するためにプランを一つずつ開く必要があり、非常に面倒。
- まっ先に改善したい要望：宿泊日を指定した上で、空いていて予約可能なプランだけをダイレクトに一覧表示・確認できるようにしてほしい。`
    },
    {
      systemName: '社内経費精算申請システム',
      surveyInput: `【一般社員アンケート】
- スマートフォンから経費申請する際、領収書の写真画像から日付や金額を読み取るOCRの誤認識が多く、結局手動で修正する手間が発生している。
- 勘定科目の分類規則が複雑で、どの科目を選択すればいいか分からない。
【承認者（管理職）アンケート】
- 外出先で承認待ちが溜まる。モバイルから手軽に1タップで内容を確認し、問題なければ即時承認できる仕組みにしてほしい。
- 規定違反 of 申請（例えば、懇親会費の上限額超過など）は自動で警告を出して弾いてほしい。`
    },
    {
      systemName: 'スマート患者予約システム',
      surveyInput: `【高齢患者アンケート】
- スマートフォンでの予約画面の文字が小さすぎて操作が難しい。
- パスワード入力や会員ログインのステップで離脱してしまう。電話番号と生年月日を入れるだけの簡単な予約方法がほしい。
- 予約完了日の前日にリマインダー通知を送ってくれないと忘れそうになる。
【クリニック院長アンケート】
- 急な休診時の自動キャンセル案内機能が必要。
- 外部の電子カルテシステムと連携し、予約完了情報がリアルタイムにカルテ側に同期されないと手動転記が発生する。`
    },
    {
      systemName: 'ECストア高速チェックアウト決済',
      surveyInput: `【一般購入者アンケート】
- カートに入れてから決済完了までのステップが長すぎる。毎回クレジットカード番号を入れるのが嫌。
- クーポンコードを入れても正しく反映されないことがある。
- 決済中の画面読み込みが遅いと、二重で注文されていないか心配になる。ワンクリック決済を導入してほしい。
【EC運用スタッフアンケート】
- クーポンコードの重複使用エラーなどのトラブルでカスタマーサポートへの問い合わせが急増している。裏側での検証を厳格にしてほしい。`
    }
  ];
  ...
  btn.addEventListener('click', () => {
    ...
    document.getElementById('system-name').value = preset.systemName;
    document.getElementById('survey-input').value = preset.surveyInput;
    
    showToast(`プリセット「${preset.systemName}」を読み込みました。`);
